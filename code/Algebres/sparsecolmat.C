...
//    $sumofcols(j)=\sum_i a_{i,j}$ 
{
  ...
  for(i=1;i<=a->DIM1();i++){
    for(k=1;k<=a->JA(i,0);k++)
      (*sumofcols)(a->JA(i,k))=(*sumofcols)(a->JA(i,k))+(*a)(i,k);
  }
 }
...
// y <- A^t x
{
  ...
  for(i=1;i<=a->DIM1();i++){
    for(k=1;k<=a->JA(i,0);k++)
      (*y)(a->JA(i,k))=(*y)(a->JA(i,k))+(*a)(i,k)*(*x)(i+lineshift);
  }
// a->JA(i,k)=J Y(J)=Y(J)+A(i,J)X(i+lineshift)
}
...
void SLSCGreg3dpg(SparseColMat *a,Vector **x,Vector **b, 
     ...
     //     WARNING work ONLY IF b and x are sampled on the SAME interval in the
     //         direction corresponding to dim1 (dimz in tomo) = NbSlice
     // arret : STOP criterion
     //         if ||*r(k)|| <= arret or if ||*r(k)||/||*r(0)||
     //           where *r(k) is the residual at iteration k.
     // verbose ==1 then the program is verbose...
...

{
  ...
  int NbSlice=(int) round(dim1*ParallelRatio);
     ...
     // verbose ==1 then the program is verbose...
{
  ...
  cout <<"  DIMENSION n "<< n <<endl;
  if( (dim1*dim2) != n)    {
    cout << " Err in SLSCGreg2D dim1,dim2: " << endl;
    cout << dim1 << " "  << dim2 << " " << dim2 << endl;
    exit(1);
  }
...
// d = -r; rkrk=||r||^2
  ...
  while(fini==FALSE) {
    ...
    ProdSparseColMatVector(a,d,vinter);
    ProdtransSparseColMatVector(a,vinter,atad);
    AddLapla2D(d,dim1,dim2, tau, atad);
    if(proj)	{
      for(i=1;i<=n;i++)
	if(out[i-1]) (*atad)(i)=0;
    }
...
//  *y=*y+tau D *x
{
  ...
  for(j=0;j<dim2;j++){
    (*y)(j+1)=(*y)(j+1)  + tau * (*x)(j+1);
    (*y)((dim1-1)*dim2+j+1)=(*y)((dim1-1)*dim2+j+1) + tau * (*x)((dim1-1)*dim2+j+1);
  }
  for(i=0;i<dim1;i++){
    (*y)(i*dim2+1)=(*y)(i*dim2+1) + tau *  (*x)(i*dim2+1);
    (*y)(i*dim2+(dim2-1)+1)=(*y)(i*dim2+(dim2-1)+1) + tau* (*x)(i*dim2+(dim2-1)+1);
  }
}
...
// a is a positive sparse rectangular (sub) matrix A
// x is a positive vector to be updated
// y contain the (sub)data
// ws is a working space of dimension of y to compute
//      ws= A x and then ws(i)=y(i)/ws(i)
//  
{
  Scalaire epsilon = 0.00000001;
  //partial proj of x  
  ProdSparseColMatVector(a,x,ax); // could be computed
  ColSumOfSparseColMat(a,sumofcols);
  // A suivre warning si ax(j)=0
  int dim1=a->DIM1();
  int dim2=a->DIM2();
  for(int i=1;i<dim1+1;i++)
    if((*ws)(i)<epsilon)
      (*ws)(i)=(*y)(i)/epsilon; // warning
    else
      (*ws)(i)=(*y)(i)/(*ws)(i);
  ProdtransSparseColMatVector(a,ws,atransws);
  for(int j=1;j<dim2+1;j++)
    (*x)(j)=(*x)(j)*(*atransws)(j)/(*sumofcols)(j);
}
