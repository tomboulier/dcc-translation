...
void NR::amoeba(Mat_IO_DP &p, Vec_IO_DP &y, const DP ftol, DP funk(Vec_I_DP &),
		int &nfunk, const int verbose)
{
	const int NMAX=4000;
	const DP TINY=1.0e-12;
	...
	for (;;) {
		...
		nfunk += 2;
		ytry=amotry(p,y,psum,funk,ihi,-1.0,verbose);
		if (ytry <= y[ilo])
			ytry=amotry(p,y,psum,funk,ihi,2.0,verbose);
		else if (ytry >= y[inhi]) {
			ysave=y[ihi];
			ytry=amotry(p,y,psum,funk,ihi,0.5,verbose);
			if (ytry >= ysave) {
				for (i=0;i<mpts;i++) {
					if (i != ilo) {
						for (j=0;j<ndim;j++)
