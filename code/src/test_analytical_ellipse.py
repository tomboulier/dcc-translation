...
class Simulator(object):
    ...
    def run(self):
        ...
        projections_array = np.zeros((self.params.Nt, self.params.imageSize))

        for time_index, time_value in enumerate(self.params.get_time_range()):
            projections_array[time_index, :] = self.ellipse.compute_projection(time_value,
                                                               self.source,
                                                               self.detector)

        # store results
        results = Results(self.params, self.source, self.detector)
        results.projections = projections_array
        return results
...
class DataConsistencyConditions(object):
    ...
    def get_virtual_source_position(self, t, v1, v2):
        ...
        # rotation matrix
        R_beta = self.compute_rotation_matrix(v1, v2)
...
if __name__ == '__main__':
    parameters = Parameters('circle_test.ini')
    results = Simulator(parameters).run()
    results.plotSinogram()

    # Plot DCC
    dcc = DataConsistencyConditions(results)

    polyproj = PolynomProjector(dcc)
    x = dcc.get_virtual_points_vector(parameters.v, parameters.v2)[10:40]
    polyproj.plot_fitting(1, parameters.v, parameters.v2, x)
